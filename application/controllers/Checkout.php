...
class Checkout extends CI_Controller {
        ...
        public function get_deliveryfee()
        {
            ...
            if($get_fee)
            {
                ...
                else
                {
                    ...
                    $data_get='₦'.number_format(floatval($get_fee[0]->delivieringchargesMerch),2,'.', ',');
            ...
            else
            {
                ...
                if($get_fee && (!empty($get_fee[0]->delivieringchargesidAdmin)))
                {
                    ...
                    else
                    {
                        ...
                        $data_get='₦'.number_format(floatval($get_fee[0]->delivieringchargesAdmin),2,'.', ',');
        ...
        if($this->session->userdata('Type')=='User') //Registerd User
        {
            ...
            if ( $_POST['paytype'] == 'delivery' )
            {
                $data_order = array(  
                                    ...
                                    //'discount'            =>   $this->input->post('cart_coupon'),
                                    //'couponcode'          =>   $this->input->post('cart_couponcode'),
                                    'deliverytype'        =>   'On Delivery ',
                                    'totalordervalue'     =>  $_SESSION['total_cart'],
                                    'status'              => 1
                                 );
                if(isset($_SESSION['s_coupon_code']))
                {
                    
                    $data_order['discount'] = $_SESSION['s_coupon_discount'];
                    $data_order['couponcode'] = $_SESSION['s_coupon_code'];
                }
                ...
                if(isset($_SESSION['s_coupon_code']))
                {
                    $coupon_data =  $this->Generic->getByFieldSingle('couponcode',$_SESSION['s_coupon_code'],$tablename='coupon');// Get url Id
                    $data_cu = array(  
                                            'accountid' =>   $_SESSION['User_id'],
                                            'couponid'  =>   $coupon_data['id'],
                                            'ordersid'  =>   $get_orderid,
                                            'status'    =>   1
                                         );
                    $insert_data_coupon = $this->Generic->add($data_cu, $tablename="couponrecodes");// insert to db and return id

                    $data_Where = array(  
                                    'couponcode'         =>  $_SESSION['s_coupon_code']
                                 );
                    $data_new = array(  
                                    'numofuserspercouponcount'  =>   $coupon_data['numofuserspercouponcount']+1
                                    );
                    $update_data_coupon = $this->Generic->editByConditions($data_new, $data_Where, $tablename="coupon");// insert to db and return id
                }
                  ...
                  //die;
                   ...
                   if(isset($_SESSION['s_coupon_code']))
                    ...
                    if(isset($_SESSION['s_coupon_code']))
                    {
                        ...
                        $insert_data_coupon = $this->Generic->add($data_cu, $tablename="couponrecodes");
                    ...
                    if($response && ($response->status = TRUE) && ($response->data->status == "success"))
                    {
                        //print("<pre>".print_r($response,true)."</pre>");die;
                         $data_order = array(  
                                            ...
                                            'paymentplan'      =>  $this->input->post('paymentplain'),
                                            'amountperpaymentplan'=>$this->input->post('amoutperpaymentplain'),
                                            'deliveryfee'       =>  $this->input->post('deliv_total_fee'),
    ...
    public function layawaypartpayment()
    {
        //print("<pre>".print_r($_SESSION,true)."</pre>");die;
        ...
        {
            $cardcode = $_POST['card_au'];
            $layawayorder = $this->Generic->getByFieldSingle('id', $_SESSION['layaway_id'], $tablename='orderlayaway'); // get merchant info
            $user = $this->Generic->getByFieldSingle('id', $_SESSION['User_id'], $tablename='accounts'); // get merchant info
                ...
                {   
                    ...
                    if($response && ($response->status = TRUE) && ($response->data->status == "success"))
                    {
                        ...
                        $data_order = array(
                                            'layawayorder'      =>   1,  
                                            'layawayorderid'    =>   $_SESSION['layaway_id'],
                                            'reference'         =>   $response->data->reference,
                                            'accountid'         =>   $_SESSION['User_id'],
                                            'totalordervalue'   =>   $this->session->userdata('total_cart'),
                                            'status'            =>  1
    ...
    public function payment_verification()
	{
                ...
                if($ordersref)
                {
                    ...
                    $this->Generic->edit($data_order,$ordersref['id'], $tablename="orders"); // update order db 
                        ...
                        else
                        {
                            ...
                            $this->jpoint($_SESSION['total_cart'],$ordersref['accountid']); // add point to user

                            if(isset($_SESSION['s_coupon_code']))
                            {
                                $coupon_data =  $this->Generic->getByFieldSingle('couponcode',$_SESSION['s_coupon_code'],$tablename='coupon');// Get url Id
                                
                                $data_cu = array(   'status'    =>   1 );
                                $data_Where_cu = array(  
                                            'ordersid'  =>   $ordersref['id']
                                         );
                                $insert_data_coupon = $this->Generic->editByConditions($data_cu, $data_Where_cu ,$tablename="couponrecodes");
        ...
        public function paymentfailure()
        {
            ...
            $data ['footer_loader']= 'included/fashionpage_included/footer';
            $data ['error_page'] = 'included/error_page_fashion';
            $data ['page_loader']= 'included/error_page_paymentfailure';
