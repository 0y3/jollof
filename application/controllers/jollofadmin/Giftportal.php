...
class Giftportal extends CI_Controller {
	...
	public function index($existing_search=0, $page=0)
	{
	    ...
	    $data['pageheader'] = "GiftPortal Products Management";
	    $data['breadCrumbs'] = '<li class="breadcrumb-item active">GiftPortal Products</li>';
	    $data['mainmenu'] = "giftportal";
        $data['submenu'] = "giftportalmanagement";
        ...
        $data['products'] = $this->product->getAllGiftportalProduct($query_params, $page,$platformid='admin');
        $data['productsAdmin'] = $this->product->getAllGiftportalProductAdmin();
        // Get record count for pagination
        $all_count = $this->product->getAllGiftportalProductCount($query_params,$platformid='admin');
        ...
        $config['base_url'] = site_url("jollofadmin/giftportal/$existing_search");
        ...
        $data ['content_file']= 'giftportal-product';
	...
	public function addform()
	{
	    ...
	    $this->session_manager->validateJollofadmin("GiftPortal::index");
	    
	    $data['cate']= $this->product->_get_fashion_cate();
        $data['brand']= $this->product->get_fashionbrands();
        
        $data['mainmenu'] = "giftportalnewproduct";
        $data['submenu']  = "giftportalmanagement";
	    $data['pageheader'] = "Add Giftportal Product";
	    $data['breadCrumbs'] = '<li class="breadcrumb-item"><a href="'.site_url("jollofadmin/giftportal").'">GiftPortal Products</a></li> <li class="breadcrumb-item active">Add GiftPortal Product</li>';

        $data ['content_file']= 'giftportal-product_new';
	    $this->load->view('jollof_admin/layout', $data);
	}
	...
	public function add()
	{
	    ...
	    // Add code to collect the product details from the form
                    ...
                    foreach($_POST['variant_check'] as $key => $value)
                    {
                        ...
                        $getcolorid = $this->Generic->getrowid($data_wherecolor, $tablename='color_tbl');// insert to db to return color id;
                             ...
                             $data_New2 = array(
                                        ...
                                        'jpoints'          => $_POST['variant_price'][$key],
                    ...
                    */
                    ...
                    if(!empty($_POST['prd_images']) )
                    {
                        ...
                        foreach($_POST['prd_images'] as $key=>$prd_image) 
                        {
                            $where = array(  
                                        'imagename'     =>  $prd_image,
                                        'adminid'  =>  $this->session->User_id//$_SESSION['rest_id'],
                                     );
            ...
            if($insert_data) 
            {
                ...
                redirect('jollofadmin/giftportal/addform', 'refresh');
	...
	// Controller function to display the edit product form
        public function editform($id=null)
        {
            ...
            $product = $this->product->getAllGiftportal($query_param,null);
            
            if($product)
            {
                $merchant = $this->Generic->getByFieldSingle('id', $_SESSION['User_id'], $tablename='admin_users'); // Get url Id
                ...
                $data['productid']= $id;
                $data['pageheader'] = "Edit GiftPortal Product";
                $data['breadCrumbs'] = '<li class="breadcrumb-item"><a href="'.site_url("jollofadmin/giftportal").'">GiftPortal Products</a></li> <li class="breadcrumb-item active">Edit GiftPortal Product</li>';
                $data['mainmenu'] = "giftportalnewproduct";
                $data['submenu']  = "giftportalmanagement";


                $data ['content_file']= 'giftportal-product_edit';
                $this->load->view('jollof_admin/layout', $data);
            }
            else
            {

                redirect('jollofadmin/giftportal', 'refresh'); 
            }
        }
        ...
        {
            $productinfo = $this->Generic->getByFieldSingle('id', $id, $tablename='products_fashion'); // Get cuisine product info by id
            $merchant = $this->Generic->getByFieldSingle('id', $productinfo['adminid'], $tablename='admin_user'); // Get url Id
            ...
            if($update_data) 
            {
                ...
                $Json_resultSave = array ('status' => '1');
                redirect('jollofadmin/giftportal/editform/'.$id, 'refresh');
        ...
        public function newimage_form($id=null) 
        {
            ...
            $data['productid']=$id;
            $this->load->view('jollof_admin/product_imageform',$data);
        }
        ...
        public function productimagesave($id=null) 
        {
            ...
            if (!empty($_FILES['product_image']['name']) )
            {
                    ...
                    $newname = $_SESSION['User_id'].'_'.$gen_code.'_product'.time();     // Random Encryp new name save to app
            ...
            if (!empty($_FILES['var_files']['name']) )
            {
                ...
                if (count($_FILES['var_files']['name'])>0 )
                {
                    foreach($_FILES['var_files']['name'] as $i => $images)
                    {
                        ...
                        $newname = $_SESSION['User_id'].'_colorimg_'.time().$i.($i+1).".".$file_extn;
                ...
                else
                {
                    ...
                    $num = $this->utility->generate_random_string(4);
                    $newname = $_SESSION['User_id'].'_colorimg_'.time().$num.".".$file_extn; // Random Encryp new name save to app
    ...
    public function savecolorimage_edit($id=null) 
    {
        $imagename   = $_POST['modal_imagename'];
        $imagename_productid = $id;
        $imagename_colorid   = $_POST['modal_colorid'];
        $adminid  = $_POST['modal_merchantid'];

        if (!empty($_FILES['invent_image']['name']) )
        {
            ...
            $exploded = explode('.',$_FILES['invent_image']['name']);
            $file_extn = strtolower(end($exploded));
            
            $file_name = "invent_image";  // name of image input from the view
            $num = $this->utility->generate_random_string(4);
            $newname = $_SESSION['User_id'].'_colorimg_'.time().$num.".".$file_extn; // Random Encryp new name save to app
            ...
            if(!$this->upload->do_upload($file_name))
            {
                $mssg = $this->upload->display_errors();
                $this->session->set_flashdata('error','error');
                $this->session->set_flashdata('message', $mssg);
                $Json_resultSave = array ('status' => '0');
                redirect('jollofadmin/giftportal/editform/'.$id, 'refresh');
            ...
            //check if there is a recode of previous image
            $check_data = $this->Generic->findByCondition($data_check, $tablename="productimages");
            if($check_data)
            {
                $data_update = array(
                                'imagename'     => $data['file_name'],
                ...
                $data_update2 = array( 'colorimage' => $data['file_name'] );
            
                $data_Where2 = array(  
                                'colorid'  => $imagename_colorid,
                                'productid'     => $id,
                             );
                // Update to db
                $update_ = $this->Generic->editByConditions($data_update, $data_Where , $tablename="productimages");

                $update_db = $this->Generic->editByConditions($data_update2, $data_Where2 , $tablename="product_qty_color_size");

                $fullpath = './assets/uploads/fashion_prod/';
                $thumbpath = './assets/uploads/fashion_prod/thumbs/';
                
                unlink($fullpath.$imagename);
                unlink($thumbpath.$imagename);
                if($update_)
                {

                    $this->session->set_flashdata('success','success');
                    $this->session->set_flashdata('message', 'Product Inventory Image Updated');
                    $Json_resultSave = array ('status' => '1');
                    redirect('jollofadmin/giftportal/editform/'.$id, 'refresh');
                }
                else
                {
                    $this->session->set_flashdata('error','error');
                    $this->session->set_flashdata('message', 'An error occur when Uploading Product Inventory Image');
                    $Json_resultSave = array ('status' => '0');
                    redirect('jollofadmin/giftportal/editform/'.$id, 'refresh');
                }
            ...
            else
            {
                ...
                $update_ = $this->Generic->editByConditions($data_update2, $data_Where2 , $tablename="product_qty_color_size");
                // insert to Product category db 
                $insert_data = $this->Generic->add($data_New,$tablename='productimages');
    ...
    public function orders($status="all", $existing_search=0, $page=0)
    {
        // Validate that the user has access to this controller
        $this->session_manager->validateJollofadmin("Giftportal::index");
        
        // Set the initial page data
        $data['pageheader'] = "Giftportal Orders Management";
        $data['breadCrumbs'] = '<li class="breadcrumb-item active">Giftportal Orders</li>';
        
        $data['mainmenu'] = "giftportal";
        $data['submenu'] = "giftportalorders";
        ...
        if(($this->input->post() && $existing_search == 0)|| ($this->input->get() && $existing_search == 0))
        {
            ...
            {
                $filterparams['giftportalorderstatus'] = $this->input->get('orderstatus');
            }
            $this->session->set_userdata(array('search_params' => $filterparams));
            $existing_search = 1;
        }
        ...
        $data['orderitems'] = $this->orderitem->getGiftportalOrders($query_params, $page);
        // Get record count for pagination
        $all_order_count = $this->orderitem->getGiftportalOrdersCount($query_params);
        
        // Load stats
        //$order_params = $query_params;
        //print("<pre>".print_r($order_params,true)."</pre>");//die;
        
        $order_params['giftportalorderstatus'] = 1; // this is to get total pending order count
        $data['pending'] = $this->order->getGiftportalOrdersCount($order_params);
        
        $order_params['giftportalorderstatus'] = 2; // this is to get total orders still in processing count
        $data['processing'] = $this->order->getGiftportalOrdersCount($order_params);
        
        $order_params['giftportalorderstatus'] = 3; // this is to get total pending order count
        $data['delivery']  = $this->order->getGiftportalOrdersCount($order_params);
        
        $order_params['giftportalorderstatus'] = 4; // this is to get total delivered order count
        $data['delivered']  = $this->order->getGiftportalOrdersCount($order_params);
        
        $order_params['giftportalorderstatus'] = 5; // this is to get total cancelled order count
        $data['canceled']  = $this->order->getGiftportalOrdersCount($order_params);
        
        $data['allorder']  = $this->order->getGiftportalOrdersCount();
        
        //print("<pre>".print_r($this->orderitem->getFashionCuisineOrders($query_params, 1, $page),true)."</pre>");die;
        // load pagenation details
        $this->load->library('pagination');
        $config['base_url'] = site_url("jollofadmin/giftportal/orders/index/$existing_search");
        ...
        $data ['content_file']= 'giftportal-order-list';
        $this->load->view('jollof_admin/layout', $data);
