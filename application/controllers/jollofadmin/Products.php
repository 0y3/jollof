...
class Products extends CI_Controller {
	...
	{
	    $this->session_manager->validateJollofadmin("Admin::product");
	...
	{
	    $this->session_manager->validateJollofadmin("Merchants::b2bproductst");
	...
	public function add()
	{
	    $this->session_manager->validateJollofadmin("Merchants::b2bproducts");
    ...
    public function editform($slug=null,$id=null)
    {
        $this->session_manager->validateJollofadmin("Merchants::b2bproducts");
        ...
        if($merchant['merchanttype']=='cuisine')
        {
            ...
            $product = $this->product->getAllProductCuisine($query_param, $page=0,$platformid='admin');

            if($product)
            {
                
                // product categories form db
                $data['cate']= $this->product->get_restaurant_category($merchant['id']);
                //print("<pre>".print_r($product,true)."</pre>");die;
                
                $data['productinfo']= $product;
                $data['productid']= $id;
                $data ['content_file']= 'product_new';
                $data['pageheader'] = "B2B's Merchant Edit Products";
                ...
                <li class="breadcrumb-item "><a href="'.site_url("jollofadmin/merchants/b2bproducts/$slug").'"> B2B Merchant Store Products </a> </li>
                <li class="breadcrumb-item active">Edit Product</li>';
                $data['mainmenu'] = "b2breview";
                $data['submenu'] = "merchantb2b";
                $data['mainmenu1'] = "products";
                $this->load->view('jollof_admin/layout', $data);
            }
            else
            {

                redirect('jollofadmin/merchants/b2bproducts/'.$slug, 'refresh'); 
            }
        }
        elseif($merchant['merchanttype']=='fashion')
        {
            // Get the product details from the db
            $query_param = array('id'=>$id);
            $product = $this->product->getAllFashion($query_param,'admin');
    ...
    {
        $productinfo = $this->Generic->getByFieldSingle('id', $id, $tablename='products_cuisine'); // Get cuisine product info by id
        $merchant = $this->Generic->getByFieldSingle('id', $productinfo['restaurantid'], $tablename='restaurants'); // Get url Id
        ...
        if(!empty($productidArr) && !empty($productchildidArr)  && !empty($productnameArr) && !empty($productpriceArr))
        {
            ...
            if($update_merges) 
            {
                ...
                $Json_resultSave = array ('status' => '1');
                //redirect('jollofadmin/products/editform/'.$merchant['slug'].'/'.$id, 'refresh');
    ...
    public function fashionedit($id=null)
    {
        $productinfo = $this->Generic->getByFieldSingle('id', $id, $tablename='products_fashion'); // Get cuisine product info by id
        $merchant = $this->Generic->getByFieldSingle('id', $productinfo['merchantid'], $tablename='restaurants'); // Get url Id
    ...
    public function deletemergesproduct($slug=null,$id=null,$editformId=null)
    {
        $this->session_manager->validateJollofadmin('Merchants::b2bproducts');
        ...
        if($_data)
        {
            ...
            $Json_resultSave = array ('status' => '1');
            redirect('jollofadmin/products/editform/'.$slug.'/'.$editformId, 'refresh');
            //echo json_encode($Json_resultSave);
            //exit();
       }
        else 
    ...
    public function addmergesproduct($slug=null)
    {
        $merchant = $this->Generic->getByFieldSingle('slug', $slug, $tablename='restaurants'); 
        $data_New = array(  
                                'restaurantid'  =>  $merchant['id'],
                                'categoryid'    =>  0,
                                'productname'   =>  $_POST["modal_additional_productname"],
                                'productprice'  =>  $_POST["modal_additional_productprice"],
                                'status'        =>  1
                             );
        $insert_product = $this->Generic->add($data_New, $tablename="products_cuisine");
        //$this->Restaurant_admin_model->_save_product($data_New, $check);// insert to db
        ...
        {
            $data_merge_id = array(
                    'parentproductid'   =>  $_POST["modal_additional_productid"],
                    'childproductid'    =>  $insert_product,
                    'price'             =>  $_POST["modal_additional_productprice"],
                 );
            $insert_merges = $this->Generic->add($data_merge_id,$tablename="productmerges");
        ...
        if (!empty($_FILES['product_image']['name']) )
            {
                ...
                $newname = $_POST['merchant_id'].'_'.$gen_code.'_product'.time();	 // Random Encryp new name save to app
             ...
             */
            $data_New = array(
                        'restaurantid'  => $_POST['merchant_id'],
                        'imagename'     => $data['file_name']
                    );
    ...
    public function savecolorimage($slug=null,$id=null) 
    {
            ...
            if (!empty($_FILES['invent_image']['name']) )
            {
                ...
                $num = $this->utility->generate_random_string(4);
                $newname = $merchantid.'_colorimg_'.time().$num.".".$file_extn; // Random Encryp new name save to app
                ...
                if(!$this->upload->do_upload($file_name))
                {
                    ...
                    $Json_resultSave = array ('status' => '0');
                    redirect('jollofadmin/products/editform/'.$slug.'/'.$id, 'refresh');
                    //print("<pre>".print_r($mssg,true)."</pre>");die; 
                }
