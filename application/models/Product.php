...
class Product extends CI_Model
{
    ...
    {   
        ...
        if(isset($params['productmerchantid']))
        {
            $this->db->where(array('products_fashion.merchantid'=>$params['productmerchantid']));
        }
        ...
        $this->db->where(array("products_fashion.giftportal" => 0));
    }
    ...
    function queryCuisineParameters($params=array())
    {
        if(isset($params['id']))
        {
            $this->db->where(array('products_cuisine.id'=>$params['id']));
        }
        ...
        // Perform checks to show merchants only orders that belong to their store
        //$this->db->where(array("products_cuisine.restaurantid" => $this->session->merchant_id));
        
        
    }

    function queryGiftportalParameters($params=array())
    {   
        // filter product by id
        if(isset($params['id']))
        {
        ...
        if(isset($params['productadminid']))
        {
            $this->db->where(array('products_fashion.adminid'=>$params['productadminid']));
        }
        ...
        // Perform checks to show merchants only orders that belong to their store
        $this->db->where(array("products_fashion.giftportal" => 1));
    }


    // Get all products from the Giftportal products table
    function getAllGiftportal($param=array(), $platformid=2)
    {
        $this->db->select('products_fashion.*, admin_users.firstname, admin_users.lastname, brands_tbl.name as brandname, brands_tbl.logo as brandlogo');
        $this->db->from('products_fashion');
        $this->db->join('admin_users', 'products_fashion.adminid = admin_users.id', "left");
        $this->db->join('brands_tbl',  'brands_tbl.id = products_fashion.productbrandid', "left");
        
        // Process any filter options if any
        $this->queryGiftportalParameters($param);
        ...
        {
            $this->db->where(array('products_fashion.isdeleted'=>0,"products_fashion.merchantid" => $this->session->merchant_id));
        }*/
    ...
    function getAllFashion($param=array(), $platformid=2)
    {
    ...
    // Get count of all cuisine products count
    function getAllCuisineCount($param=array(),$platformid=1)
    {
        $this->db->from('products_cuisine');
         $this->db->join("categories", "categories.id = products_cuisine.categoryid",'left OUTER');
        $this->db->join("restaurants", "restaurants.id = products_cuisine.restaurantid ", 'left');
        
        // Process any filter options if any
        $this->queryCuisineParameters($param);
        // Clause to only fetch data with deletedat field set to null

        if($platformid=='admin')
        {
            $this->db->where(array("categories.categorystatus" => 1,"products_cuisine.status"=>1,"products_cuisine.isdeleted" => 0));
        }
        else
        {
            $this->db->where(array("categories.categorystatus" => 1,"products_cuisine.status"=>1,"products_cuisine.isdeleted" => 0,"products_cuisine.restaurantid" => $this->session->merchant_id));
        }
    ...
    public function getAllProductCuisine($param=array(), $limit_start,$platformid=1)
    {

        $this->db->select("products_cuisine.*,categories.id as category_id,categories.categoryname,categories.arrangecategory,categories.slug as category_slug, restaurants.companyname, restaurants.slug as companynameslug");
        $this->db->from("products_cuisine");
        $this->db->join("categories", "categories.id = products_cuisine.categoryid",'left OUTER');
        $this->db->join("restaurants", "restaurants.id = products_cuisine.restaurantid ", 'left');
        ...
        if($query->num_rows() > 0)
        {
            ...
            foreach($rows as $row)
            {
                $row['added_product'] =   $this->getProductmergesByid($row['id']);
                $get_prod[] = $row;
            }
    ...
    Public function getProductmergesByid($by_id)
    {
        $this->db->select("productmerges.*,products_cuisine.productname as childproductname");
        $this->db->from('productmerges');
        $this->db->join( "products_cuisine", "products_cuisine.id = productmerges.childproductid ", 'left');
        $this->db->where("productmerges.parentproductid", $by_id);
        $this->db->where("productmerges.status", 1);
        $this->db->where("productmerges.isdeleted", 0);
        $this->db->order_by("productmerges.id", 'ASC');
        $query = $this->db->get();
        return $query->result_array();
    }

    public function getAllProductCuisineoMerchant() 
    {
        $this->db->distinct();
    ...
    public function getAllProduct($param=array(), $limit_start,$platformid=2)
    {
        ...
        $this->db->select("productimages.imagename, products_fashion.*, restaurants.companyname, restaurants.slug as companynameslug, categories_fashion.id AS category_id, 
            categories_fashion.categoryparentid, categories_fashion.categoryname, SUM(product_qty_color_size.productinstock) AS sum_productinstock,SUM(product_qty_color_size.productsold) AS sum_productsold,
    ...
    // Get count of all fashion products count
    function getAllProductCount($param=array(),$platformid=2)
    {
        $this->db->select('products_fashion.*');
        $this->db->from('products_fashion');
        $this->db->join("product_qty_color_size", "product_qty_color_size.productid = products_fashion.id",'left');
        $this->db->join("restaurants", "restaurants.id = ".$this->DB_product_fas.".merchantid ", 'left');
        $this->db->join("product_fashion_cate_assign", "product_fashion_cate_assign.product_fasid = products_fashion.id", 'left');
    ...
    public function getAllProductMerchant() 
    {
        $this->db->distinct();
        $this->db->select('restaurants.id, restaurants.companyname');
        $this->db->from("products_fashion");
        $this->db->join("restaurants", "restaurants.id = ".$this->DB_product_fas.".merchantid ", 'left');
        $this->db->where(array("products_fashion.isdeleted" => 0,"products_fashion.giftportal" => 0));
        $this->db->order_by("restaurants.companyname");
        $query = $this->db->get();
        if ($query->num_rows() > 0){
            return $query->result_array();
    ...
    // Get count of all fashion Giftportal products
    Public function getAllGiftportalProduct($param=array(), $limit_start,$platformid=2)
    {
        
        $this->db->select("productimages.imagename, products_fashion.*, admin_users.firstname, admin_users.lastname, categories_fashion.id AS category_id, 
            categories_fashion.categoryparentid, categories_fashion.categoryname, SUM(product_qty_color_size.productinstock) AS sum_productinstock,SUM(product_qty_color_size.productsold) AS sum_productsold,
            product_qty_color_size.jpoints, min(product_qty_color_size.price) AS min_price, max(product_qty_color_size.price) AS max_price, 
            count(DISTINCT product_qty_color_size.id) AS variant");
        $this->db->from("products_fashion");
        $this->db->join("product_qty_color_size", "product_qty_color_size.productid = products_fashion.id",'left');
        $this->db->join("admin_users", "admin_users.id = ".$this->DB_product_fas.".adminid ", 'left');
        $this->db->join("product_fashion_cate_assign", "product_fashion_cate_assign.product_fasid = products_fashion.id", 'left');
        $this->db->join("categories_fashion", "categories_fashion.id = product_fashion_cate_assign.cat_fasid", 'left');
        $this->db->join("    
                (
    ...
    // Get count of all fashion Giftportal products count
    function getAllGiftportalProductCount($param=array(),$platformid=2)
    {
        $this->db->select('products_fashion.*');
        $this->db->from('products_fashion');
        $this->db->join("product_qty_color_size", "product_qty_color_size.productid = products_fashion.id",'left');
        ...
        if($platformid=='admin')
        {
            $this->db->where(array("products_fashion.isdeleted" => 0,"products_fashion.giftportal" => 1));
        }
        else
        {
            //$this->db->where(array("products_fashion.isdeleted" => 0,"products_fashion.merchantid" => $this->session->merchant_id));
    ...
    public function getAllGiftportalProductAdmin() 
    {
        $this->db->distinct();
        $this->db->select('admin_users.id, admin_users.firstname,admin_users.lastname');
        $this->db->from("products_fashion");
        $this->db->join("admin_users", "admin_users.id = ".$this->DB_product_fas.".adminid ", 'left');
        $this->db->where(array("products_fashion.isdeleted" => 0,"products_fashion.giftportal" => 1));
        $this->db->order_by("admin_users.firstname");
        $query = $this->db->get();
        if ($query->num_rows() > 0){
            return $query->result_array();
    ...
    public function get_fashionbrands()
    {
        $this->db->select("*");
    ...
    function deleteFashionCateByProductId($id)
    {
        $this->db->where(array('product_fasid'=>$id));
        $status = $this->db->delete('product_fashion_cate_assign');
        return $status;
    }
    ...
    public function _getcolorid($id) 
    {        
        $_id = $this->db->get_where('size_tbl' , array( 'sizecode' =>  $id, 'isdeleted' =>0 ))->row_array(); 
        if($_id == TRUE) { return $_id['id']; }
        else {return FALSE; }
    }
    
    public function get_restaurant_category($id=null)
    {
        $this->db->select("*");
