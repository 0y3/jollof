                    ...
                    <div class="block full">
                        ...
                        <div class="block-title">
                            ...
                            <div class="block-options pull-right">
                                ...
                                <a href="<?= site_url('restaurant_admin/addcategory') ?>" class="btn btn-sm btn-default" data-toggle="modal" data-tooltip="tooltip" data-target="#modal_cate" data-container="modal_cate" title="Add New Category" style="background-color:#1bbae1; color: #FFF">
...
<script >
    //$('#order_datatable').dataTable(); 
      var dataTable = $('#order_datatable').DataTable({  
           ...
           "order":[],  
           "ajax":{  
                url:"<?= site_url('restaurant_admin/get_cate'); ?>",  
                type:"POST"  
           },  
           "columnDefs":[  
                {  
...
<script>
    ...
    $('.selectstatus').on('change',function(){
            ...
            if(statusID){
                $.ajax({
                    type:'POST',
                    url:'<?= site_url('restaurant_admin/cat_statusupdate') ?>',
                    dataType: 'json',
                    data:{
                            status:statusID,
                            cat_id:cat_id_
...
<script>
    ...
    $('#cat_order_save').click(function (e) {
            ...
            var cat_id_   = $('[name="new_cat_order"]').data('cat_id'); // gets value
                ...
                $.ajax({
                    type:'POST',
                    url:'<?= site_url('restaurant_admin/cat_orderupdate') ?>',
                    dataType: 'json',
                    data:{
                            order:new_order,
                            cat_id:cat_id_
