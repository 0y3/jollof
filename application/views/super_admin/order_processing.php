...
<script >
    ...
    function load_data(restid)
    {
      var dataTable = $('#order_datatable').DataTable({  
           ...
           "order":[],  
           "ajax":{  
                url:"<?= site_url('super_admin/get_order_by/2'); ?>",
                type:"POST",
                data:{rest_id:restid}
           },  
           "columnDefs":[  
        ...
        var ord_id_   = $(this).data('get'); // gets value
            ...
            $.ajax({
                ...
                success:function(html){
                    ...
                    /*htmldata = "";
                    htmldata += "<select name=\"rest_category\" id=\"rest_category\" class=\"selectpicker\" data-live-search=\"true\" show-tick  data-size='8' title=\"Restaurants Search\" data-width=\"90%\" >";
                    htmldata += "<option value=\"\">Restaurants Search </option>";
                    htmldata += " <?php 
                                                $processing = $this->Super_admin_model->_get_processing_data_fromorder();
            ...
            $.ajax({
                ...
                success:function(html){
                    ...
                    if(html.status === '1'){
                        notice();
                        dataTable.ajax.reload();
                        $('.conte').load("<?= site_url('super_admin/view_processing_order') ?>");
                    }
                    else
                    {
                        noticefales();
